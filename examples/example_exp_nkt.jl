...
**NKT Photonics Basik E15** commercial Fiber laser
...
begin
	...
	f_bw = 0.96 * 6e6 # filter bandwidth of bandpass filter around beat note
	l_delay_est = 530.0
	
	file = "exp_data/E15_selfhet.trc"
	...
	p1 = Plots.plot(SFM.ds(f_signal, p_signal),
			   yscale=:log10, xlabel="Frequenc/ Hz",
...
begin
	...
	ONLY_FIRST_N_PEAKS = 6;
	n_avg_param_est = 250;
	# train_mask_filter_func = (f, p) -> (f .> 0.5/tau_est) .& (f .< f_bw/2) ;
	train_mask_filter_func = (f, p) -> (f .> 0.5/tau_est) .& (f .< 2e6) ;
...
begin
	if run_all
		...
		lambdas = T.(10.0 .^ range(-4.0, 0.0, length=n_krr_grid))
		if xlog
